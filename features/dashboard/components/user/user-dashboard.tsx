...
import { useUserAnalytics, useUserStats } from "../../queries/dashboard.queries";
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
        {statCards.map((card, index) => (
          <motion.div
            ...
            className="rounded-3xl border bg-white dark:bg-slate-900 p-6 shadow-sm hover:shadow-md transition-all border-b-4 border-b-transparent hover:border-b-green-600"
          >
            <div className={`w-12 h-12 rounded-2xl flex items-center justify-center mb-4 ${card.bgColor}`}>
            ...
            <div>
              ...
              <div className="flex items-baseline gap-2 mt-1">
                ...
                {card.target && (
                  <span className="text-sm text-slate-400 font-medium">{card.target}</span>
                )}
              </div>
            </div>
          </motion.div>
      ...
      <div className="grid gap-8 lg:grid-cols-3">
        ...
        >
          <div className="flex items-center justify-between mb-8">
            <div>
              ...
              <p className="text-sm text-slate-500">Your intake over the last 7 days</p>
            </div>
          </div>
        ...
        >
          <h3 className="text-xl font-bold text-slate-900 dark:text-white mb-2">Macro Distribution</h3>
          <p className="text-sm text-slate-500 mb-8">Weekly nutrient average (g)</p>
          
          <div className="flex-1 min-h-[250px]">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={analytics?.macroBreakdown || []} layout="vertical" barSize={30}>
          ...
          <div className="mt-6 pt-6 border-t border-slate-100 dark:border-slate-800 grid grid-cols-3 gap-2">
            {(analytics?.macroBreakdown || []).map((item, index) => (
              <div key={item.name} className="text-center">
                <p className="text-[10px] uppercase tracking-wider font-bold text-slate-400 mb-1">{item.name}</p>
                <p className="text-lg font-bold" style={{ color: COLORS[index % COLORS.length] }}>{item.value}g</p>
              </div>
            ))}
          </div>
        </motion.div>
      ...
      <div className="grid gap-8 lg:grid-cols-2">
        ...
        >
          <div className="flex items-center justify-between mb-8">
            <h3 className="text-xl font-bold text-slate-900 dark:text-white">Recent Meals</h3>
            <Link href="/logs" className="text-sm font-semibold text-[#065E32] hover:underline flex items-center">
              ...
              dailyLog.meals.map((log: any) => (
                <div key={log.id} className="flex items-center justify-between group cursor-pointer">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <p className="font-bold text-slate-900 dark:text-white">{log.foodName || "Meal Log"}</p>
                      <p className="text-sm text-slate-500">{new Date(log.time).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })} • {log.mealType || "Snack"}</p>
                    </div>
                  </div>
                  <p className="font-bold text-slate-900 dark:text-white">{log.calories} kcal</p>
                </div>
              ))
            ) : (
        ...
        >
          ...
          <div className="relative z-10 h-full flex flex-col">
            ...
            <p className="text-green-50 text-lg leading-relaxed mb-8 flex-1">
              "Based on your recent activity, adding more protein to your breakfast could help sustain your energy levels throughout the morning. Try adding a boiled egg or some Greek yogurt!"
            </p>
