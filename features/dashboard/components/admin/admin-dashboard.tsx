...
import { useAdminAnalytics, useAdminStats } from "../../queries/dashboard.queries";
import { 
  Users, 
  BookOpen, 
  Calendar, 
...
const COLORS = ["#065E32", "#44B74C", "#FFBB28", "#FF8042", "#0088FE", "#00C49F"];
    ...
    <div className="space-y-8">
      ...
      <div className="flex flex-col gap-2">
        ...
        <p className="text-slate-500 dark:text-slate-400 text-lg">
          Welcome back! Here's what's happening with NutriSync today.
        </p>
      </div>
      ...
      <div className="grid gap-8 lg:grid-cols-3">
        ...
        >
          <h3 className="text-xl font-bold text-slate-900 dark:text-white mb-2">Cuisine Mix</h3>
          <p className="text-sm text-slate-500 mb-8">Popular recipe categories</p>
          ...
          <div className="mt-6 space-y-3">
            {(analytics?.cuisineDistribution || []).slice(0, 4).map((item, index) => (
          ...
          {[1, 2, 3].map((i) => (
            <div key={i} className="flex items-center justify-between pb-6 border-b border-slate-100 dark:border-slate-800 last:border-0 last:pb-0">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <p className="font-semibold text-slate-900 dark:text-white">System Backup Completed</p>
                  <p className="text-sm text-slate-500">2 minutes ago • Auto-triggered</p>
                </div>
              </div>
              <span className="px-3 py-1 bg-green-50 text-green-600 dark:bg-green-900/20 dark:text-green-400 text-xs font-bold rounded-lg uppercase">Success</span>
            </div>
          ))}
        </div>
      </motion.div>
